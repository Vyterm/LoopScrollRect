...
namespace SG
{
    ...
    class Pool
    {
        ...
        private void AddObjectToPool(PoolObject po)
        {
            ...
            po.gameObject.name = poolObject.name;
...
#if UNITY_EDITOR
                Debug.Log(string.Format("Growing pool {0}: {1} populated", poolObject.name, increaseSize));
#endif
                if (increaseSize > 0)
                {
        ...
        {
            if (poolObject.Equals(po.poolObject))
            {
                objectsInUse--;
            ...
            {
                Debug.LogError(string.Format("Trying to add object to incorrect pool {0} {1}", po.poolObject.name, poolObject.name));
            }
        }
    }
}