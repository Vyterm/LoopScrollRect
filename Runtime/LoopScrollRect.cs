...
namespace UnityEngine.UI
{
    ...
    {
        public void InitLoop(GameObject item, int loopMax, Action<Transform, int> callback, System.Action<Transform> cellReturn)
        {
            prefabSource.prefab = item;
            prefabSource.callback = cellReturn;
            dataSource = callback;
            totalCount = loopMax;
            RefillCells();
        }
        ...
        {
            prefabSource.callback = null;
            dataSource = null;
        }

        //==========LoopScrollRect==========
        [Tooltip("Prefab Source")]
        protected LoopScrollPrefabSource prefabSource = new LoopScrollPrefabSource();
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        dataSource.Invoke(content.GetChild(i), itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        protected RectTransform GetFromTempPool(int itemIdx)
        {
            ...
            dataSource.Invoke(nextItem, itemIdx);
            return nextItem;
        }
